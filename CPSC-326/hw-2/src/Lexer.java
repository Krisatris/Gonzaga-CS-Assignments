  ...
  public Token nextToken() throws MyPLException {
    ...
    // integer and double values
      ...
      if(isSymbol(peek(), ' ') || isSymbol(peek(), '\n') || peek() == -1) {
        ...
        error("missing decimal digit in double value '" + resultStr + "'", line, column - offset);
      }
    }
    ...
    // anything left is an ID or unexpected character
    // if the next char is whitespace, newline, or EOF the identifier is finished
    if(isWhitespace(peek()) || isSymbol(peek(), '\n') || peek() == -1) {
      return new Token(TokenType.ID, resultStr, line, column - offset);
    }
    //check for nums and underscores
    else if(isSymbol(peek(), '_') || isDigit(peek())) {
      currChar = read();
    ...
    System.out.println("identifier token reached");
    return new Token(TokenType.ID, resultStr, line, column - offset);
  }
