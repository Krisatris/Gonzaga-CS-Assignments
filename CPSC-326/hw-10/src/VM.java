...
class VM {
  ...
  // thrown object storage
  public int thrownObject = -1;

  // name of type of thrown object
  ...
  public void run() throws MyPLException {
    ...
    while (frame != null && frame.pc < frame.instructions.size()) {
      ...
      //------------------------------------------------------------
        ...
        Integer oid = (int)frame.operandStack.pop();
        ensureNotNil(frame, oid);
        // ensure oid points to legit defined object
        if(heap.get(oid) == null) {
          error("object id not valid", frame);
        }
        thrownObject = oid;
        String thrownType = (String)frame.operandStack.pop();
        thrownTypeName = thrownType;
      }

      else if(instr.opcode() == OpCode.CATCH) {
        /*
        Integer oid = (int)frame.operandStack.pop();
        if(oid != frame.thrownObject) {
          error("caught exception does not match thrown exception", frame);
        }
        */
        frame.operandStack.push(thrownObject);
        //thrownObject = -1;
        frame.operandStack.push(thrownTypeName);
        //thrownTypeName = "";
      }
