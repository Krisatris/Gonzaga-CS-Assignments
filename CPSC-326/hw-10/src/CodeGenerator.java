...
public class CodeGenerator implements Visitor {
  ...
  public void visit(ThrowStmt node) throws MyPLException {
    currFrame.instructions.add(VMInstr.PUSH(node.typeName));
    int index = varMap.get(node.varName.lexeme());
    currFrame.instructions.add(VMInstr.LOAD(index));
    currFrame.instructions.add(VMInstr.THROW());
  }
  ...
  public void visit(CatchStmt node) throws MyPLException {
    currFrame.instructions.add(VMInstr.CATCH());
    varMap.put(node.varName.lexeme(), currVarIndex);
    // check the type name of thrown object and catch are the same (similar to if stmt)
    currFrame.instructions.add(VMInstr.PUSH(node.typeName.lexeme()));
