...
public class ASTParser {
  ...
  private boolean isStmt() {
    return match(TokenType.ID) || match(TokenType.VAR) || match(TokenType.IF) ||
      ...
      match(TokenType.RETURN) || match(TokenType.DELETE) || //match(TokenType.CATCH) ||
      match(TokenType.TRY) || match(TokenType.THROW);
  }
  ...
  private void params(FunDecl fDeclNode) throws MyPLException {
    ...
    if(isPrimitiveType() || match(TokenType.ID)) { // <dtype>
      ...
      while(match(TokenType.COMMA)) {
        FunParam fParamNode2 = new FunParam();
        advance();
        if(isPrimitiveType() || match(TokenType.ID)) { // <dtype>
          fParamNode2.paramType = currToken;
          advance();
        }
        else {
        ...
        if(match(TokenType.ID)) {
          fParamNode2.paramName = currToken;
          advance();
        }
        else {
          error("expected parameter name");
        }
        fDeclNode.params.add(fParamNode2);
      }
    }
  }
  ...
  private void stmt(List<Stmt> stmtsListNode) throws MyPLException {
    ...
    if(match(TokenType.VAR)) {
      ...
      ThrowStmt stmtNode = new ThrowStmt();
      throw_stmt(stmtNode);
      stmtsListNode.add(stmtNode);
    }
    else if(match(TokenType.TRY)) {
      TryStmt stmtNode = new TryStmt();
      try_stmt(stmtNode);
      stmtsListNode.add(stmtNode);
    }
    else if(match(TokenType.CATCH)) {
      error("catch cannot be used without proceeding try stmt");
    }
      ...
      if(match(TokenType.DOT) || match(TokenType.ASSIGN)) { // <assign_stmt>
        ...
        if(isExpr()) { // <args>
          ...
          while(match(TokenType.COMMA)) {
            Expr longNode = new Expr();
            advance();
            expr(longNode);
            stmtNode.args.add(longNode);
          }
        }
        eat(TokenType.RPAREN, "expected ')");
        stmtsListNode.add(stmtNode);
  ...
  private void throw_stmt(ThrowStmt tStmtNode) throws MyPLException {
    // <throw_stmt> ::= THROW ID
    eat(TokenType.THROW, "expected throw keyword");
    if(match(TokenType.ID)) {
      tStmtNode.varName = currToken;
      advance();
    }
    else {
      error("expected identifier");
  ...
  private void try_stmt(TryStmt tStmtNode) throws MyPLException {
    // <try_stmt> ::= TRY LBRACE <stmts> RBRACE <catch_stmt>
    eat(TokenType.TRY, "expected try keyword");
    eat(TokenType.LBRACE, "expected left brace");
    List<Stmt> stmtsNode = new ArrayList<>();
    while(isStmt()) {
      stmt(stmtsNode);
    }
    ...
    if(match(TokenType.CATCH)) {
      CatchStmt stmtNode = new CatchStmt();
      catch_stmt(stmtNode);
      tStmtNode.catchStmt = stmtNode;
    }
    else {
      error("expected catch keyword after try statement");
    }
    tStmtNode.stmts = stmtsNode;
  }

  private void catch_stmt(CatchStmt cStmtNode) throws MyPLException {
    // <catch_stmt> ::= CATCH TYPE ID LBRACE <stmts> RBRACE
    advance(); // already checked catch keyword in try_stmt()
    if(match(TokenType.ID)) {
      cStmtNode.typeName = currToken;
      advance();
    }
    else {
    ...
    eat(TokenType.RBRACE, "expected right brace");
    cStmtNode.stmts = stmtsNode;
    if(match(TokenType.CATCH)) {
      CatchStmt anotherCatch = new CatchStmt();
      catch_stmt(anotherCatch);
      cStmtNode.otherCatch = anotherCatch;
    }
  }
