...
public class StaticChecker implements Visitor {
      ...
      if(node.expr.first instanceof SimpleTerm) {
        ...
        if(!(s.rvalue instanceof NewRValue)) {
          if(s.rvalue instanceof IDRValue) {
            ...
            if(i.path.size() == 1) {
              Token firstToken = i.path.get(0);
              if(!symbolTable.nameExistsInCurrEnv(firstToken.lexeme())) {
          ...
          else if(!(s.rvalue instanceof CallExpr)) {
            error("expecting variable, path, or new", getFirstToken(node.expr));
          }
        }
      } 
  ...
  public void visit(ThrowStmt node) throws MyPLException {
    // check thrown variable is of valid UDT
    String varName = node.varName.lexeme();
    String typeName = symbolTable.get(varName);
    Set<String> udtSet = typeInfo.types();
    if(!udtSet.contains(typeName)) {
      error("throw must be used with a user-defined type", node.varName);
    }
    // typeName is checked and tied to varName, save it in the object
    node.typeName = typeName;
  }

  public void visit(TryStmt node) throws MyPLException {
    // check stmts
    symbolTable.pushEnvironment();
    for(Stmt stmtNode : node.stmts) {
      stmtNode.accept(this);
    ...
    symbolTable.popEnvironment();
    node.catchStmt.accept(this);
  }

  public void visit(CatchStmt node) throws MyPLException {
    // check type name in catch stmt is a valid type
    String typeName = node.typeName.lexeme();
    if(!symbolTable.get(typeName).equals("type") || symbolTable.get(typeName) == null) {
      String s = typeName + " is not a user defined type";
      error(s, node.typeName);
    }
    // check that varName catch is named is not already in use
    String varName = node.varName.lexeme();
    if(symbolTable.nameExistsInCurrEnv(varName)) {
      String s = varName + " is already defined in current environment";
      error(s, node.varName);
    }
    // check that the type and var name are of a valid UDT
    Set<String> udtSet = typeInfo.types();
    if(!udtSet.contains(typeName)) {
      error("catch must be used with a user-defined type", node.typeName);
    }
    // check stmts
    symbolTable.pushEnvironment();
    symbolTable.add(node.varName.lexeme(), node.typeName.lexeme());
    ...
    symbolTable.popEnvironment();
    if(node.otherCatch != null) {
      node.otherCatch.accept(this);
    }
  }
